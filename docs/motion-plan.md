...
**Remaining, all asset-blocked:** hero motion (needs the new hero UI via /herolab — `data-split` + lerp helper land with it), lerp pointer-follow (part of hero), audio-demo meter map (needs bounced A/B audio files; section currently hidden), testimonial/press/gallery-crossfade reveals (wired, dormant until data).
